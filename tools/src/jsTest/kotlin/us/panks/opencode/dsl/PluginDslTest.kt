...
@Serializable
private data class SerializedPathInput(
    val path: String,
    val recursive: Boolean = false,
)

@Serializable
private data class RecursiveInput(val child: RecursiveInput? = null)
...
class PluginDslTest {
    ...
    @Test
    fun infersSchemaAndDecodesSerializableInput(): Promise<Unit> {
        var transformCallback: dynamic = null
        var received: SerializedPathInput? = null
        val plugin = opencodePlugin("example.serialized-tool") {
            tools {
                transform {
                    tool<SerializedPathInput>("inspect_path") {
                        description = "Inspect a path"
                        execute { input, _ ->
                            received = input
        ...
        return plugin.setup(context)
            ...
            .then<dynamic> {
                ...
                assertEquals("object", definition.input.type as String)
                assertEquals("string", definition.input.properties.path.type as String)
                assertEquals("boolean", definition.input.properties.recursive.type as String)
                assertEquals(arrayOf("path").toList(), definition.input.required.unsafeCast<Array<String>>().toList())
                assertEquals(false, definition.input.additionalProperties as Boolean)

                val input: dynamic = js("({ path: '/tmp/demo' })")
                definition.execute(input, js("({})"))
            }
            .then<Unit> { result ->
                assertEquals(SerializedPathInput("/tmp/demo"), received)
                assertEquals("PATH: /tmp/demo", result.content as String)
            }
    }

    @Test
    fun rejectsRecursiveSerializableSchemaClearly(): Promise<Unit> {
        var transformCallback: dynamic = null
        val plugin = opencodePlugin("example.recursive-schema") {
            tools {
                transform {
                    tool<RecursiveInput>("recursive") {
                        description = "Recursive input"
                        execute { _, _ -> toolResult(content = "unused") }
                    }
                }
            }
        }
        ...
        return plugin.setup(context)
            .unsafeCast<Promise<dynamic>>()
            .then<Unit> {
                val editor: dynamic = js("({ add: () => undefined })")
                val failure = assertFailsWith<IllegalStateException> { transformCallback(editor) }
                assertTrue(failure.message.orEmpty().contains("Recursive serial descriptor"))
            }
    }
}
